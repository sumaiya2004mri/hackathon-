...
import EmergencyTriageDashboard from "./components/EmergencyTriageDashboard";
import { AuthScreen } from "./components/AuthScreen";
import { PregnancyModule } from "./components/PregnancyModule";
import { FemaleHealthModule } from "./components/FemaleHealthModule";
...
export default function App() {
  ...
  // Require an account when accounts are configured. If Firebase env vars
  // aren't set (e.g. local dev before setup), fall through and let the
  // core triage flow work without auth rather than hard-blocking the app.
  ...
  // The clinical dashboard is a self-contained, full-screen experience
  // (its own dark theme, header, and layout) — render it outside the
  // normal light-theme app shell rather than nesting it.
  if (view === "dashboard") {
    return (
      <div className="relative">
        <button
          onClick={() => setView("home")}
          className="fixed left-4 top-4 z-50 rounded-full border border-slate-700 bg-slate-800/90 px-4 py-2 text-xs font-semibold text-slate-200 backdrop-blur transition-colors hover:bg-slate-700"
        >
          ← Back to Emergency AI
        </button>
        <EmergencyTriageDashboard />
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1">
        ...
        {view === "pregnancy" && <PregnancyModule onBack={() => setView("home")} />}
        {view === "female-health" && (
          <FemaleHealthModule onBack={() => setView("home")} />
        )}
      </main>
